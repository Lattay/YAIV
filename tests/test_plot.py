...
BANDS_CASES = [
    SimpleNamespace(
        ...
        kpath="vasp/RESULTS/KPATH",             # optional but provided here
        lattice="vasp/RESULTS/POSCAR",          # optional but provided here
        window_eV=1.5,
    ),
#    SimpleNamespace(
#        name="vasp_eigenval_kpath_poscar",
#        bands="vasp/RESULTS/EIGENVAL",
#        kpath="vasp/RESULTS/KPATH",
#        lattice="vasp/RESULTS/POSCAR",
#        window_eV=1.5,
#    ),
#    SimpleNamespace(
#        name="vasp_procar_kpath_poscar",
#        bands="vasp/RESULTS/PROCAR",
#        kpath="vasp/RESULTS/KPATH",
#        lattice="vasp/RESULTS/POSCAR",
#        window_eV=1.5,
#    ),
#    SimpleNamespace(
#        name="qe_xml_bands_in_dyn",
#        bands="qe/tmp/Si.xml",
#        kpath="qe/results/bands.in",
#        lattice="qe/results/Si.dyn1",
#        window_eV=1.5,
#    ),
]
...
@pytest.mark.mpl_image_compare(style="default", tolerance=15, savefig_kwargs={"bbox_inches": "tight"})
...
#
#    fig, ax = plt.subplots()
#    plot.DOS(bands, ax=ax, window=1.0 * ureg.eV, order=0, precision=3.0, fill=True, alpha=0.4)
#
#    # Structural checks
...
## ---------- Bands + DOS panel ----------
#@pytest.mark.parametrize("case", BANDS_CASES, ids=[c.name for c in BANDS_CASES])
#@pytest.mark.mpl_image_compare(style="default", tolerance=18, savefig_kwargs={"bbox_inches": "tight"})
#def test_bands_dos_panel_image_and_structure_cases(data_dir, require, case):
#    bands = build_electron_bands(case, data_dir, require)
#
#    fig = plt.figure(figsize=(6, 3))
#    ax_b, ax_d = plot.bandsDOS(bands, fig=fig, window=getattr(case, "window_eV", 1.5) * ureg.eV)
#
...
#    assert x0 == pytest.approx(0.0) and x1 == pytest.approx(1.0)
#    # y=0 line present
#    found_zero = any((ln.get_ydata()[0] == 0 and ln.get_ydata()[-1] == 0) for ln in ax.lines)
#    assert found_zero
#
...
#
#    fig = plt.figure(figsize=(6, 3))
#    ax_b, ax_d = plot.phononDOS(pb, fig=fig, window=None)
#
#    assert len(ax_b.get_lines()) >= 1
#    x0, x1 = ax_b.get_xlim()
#    assert x0 == pytest.approx(0.0) and x1 == pytest.approx(1.0)
