...
def E_cut(file,HSP=None,title=None,border=0.1,axis=None,save_interp=True,save_as=None,figsize=None,colormap='plasma',text_color='black'):
    """Plots and interpolates the surface  generated by WannierTools
    file = dos.dat file from WT or the output of __load_surfdos_data
    HSP = The labels for the "high symmetry points" at (0,0), (0,0.5),(0.5,0.5) and (0.5,0). At that order...
    title = string with the title for the plot
    border = How much cell to expand beyond the BZ in the plot
    axis = Matplotlib axis in which you want to plot
    save_interp = Boolean controlling if you want to save the interpolation (to save time in future plots)
    save_as = string with the saving file and extensions
    figsize = (int,int) => Size and shape of the figure
    ...
    if border > 0:
        ...
        bx=2*x/nx
        by=2*y/ny
        Z=np.vstack((Z[-y:,:],Z,Z[:y,:]))
        Z=np.hstack((Z[:,-x:],Z,Z[:,:x]))
        #BZ limits
        ls,c,alpha,lw='--','black',0.7,0.7
        ax.plot([xmin,xmin],[ymin,ymax],linestyle=ls,color=c,linewidth=lw)
        ax.plot([xmax,xmax],[ymin,ymax],linestyle=ls,color=c,linewidth=lw)
        ax.plot([xmin,xmax],[ymin,ymin],linestyle=ls,color=c,linewidth=lw)
        ax.plot([xmin,xmax],[ymax,ymax],linestyle=ls,color=c,linewidth=lw)
    else:
        bx,by=0,0

    pos=ax.imshow(Z,interpolation='bilinear',aspect='equal',cmap=colormap,
                  origin='lower',extent=(xmin*(1+bx),xmax*(1+bx),ymin*(1+by),ymax*(1+by)))
    # draw gridlines
    ...
    if HSP!=None:
        ...
        ax.text(x=-2*xs, y=-ys, s=HSP[0], fontsize=16,color=c)
        ax.plot(0,ymax,'o', markersize=2,color=c) 
        ax.text(x=-xs, y=ymax+ys, s=HSP[1], fontsize=16,color=c)
        ax.plot(xmax,ymax,'o', markersize=2,color=c) 
        ax.text(x=xmax+xs, y=ymax-ys, s=HSP[2], fontsize=16,color=c)
        ax.plot(xmax,0,'o', markersize=2,color=c) 
        ax.text(x=xmax+xs, y=-ys, s=HSP[3], fontsize=16,color=c)
