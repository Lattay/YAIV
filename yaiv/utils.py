...
def grep_lattice(file, alat=False, filetype=None):
    ...
    if filetype == "qe_ph_out":
        ...
        warnings.filterwarnings(
            "ignore",
            message="Non-collinear spin is not yet implemented. Setting magmom to x value.",
        )
        try:
            data = io.read(file)
    ...
    if filetype[:2] == "qe":
        for line in reversed(list(lines)):
            if re.search("!", line):
                ...
                U_ewald = float(l[3])
    if meV == True:
        F, TS, U, U_one_electron, U_h, U_xc, U_ewald = [
            X * const.Ry2eV * 1000
            for X in [F, TS, U, U_one_electron, U_h, U_xc, U_ewald]
        ]
    return F, TS, U, U_one_electron, U_h, U_xc, U_ewald
