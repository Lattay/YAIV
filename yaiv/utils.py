...
def grep_filetype(file: str) -> str:
    """
...
def grep_electrons(file: str) -> int:
    """
...
def grep_lattice(file: str, alat: bool = False) -> np.ndarray:
    """
...
def grep_stress_tensor(file: str) -> np.ndarray:
    """
...
def grep_ticks_labels_KPATH(file):
    ...
    for i in range(len(labels)):
        if "Gamma" in labels[i]:
            labels[i] = (
                labels[i].split("Gamma")[0] + "\Gamma" + labels[i].split("Gamma")[1]
            )
        labels[i] = "$" + labels[i] + "$"
    return ticks, labels
